...
const CartItem = ({ product, productsLoading, quantity, cartItemsUpdater }) => {
  ...
  const handleDialogResult = (action, itemId) => {
    if (action === "confirmed") {
      // const deleteProduct = products.find((product) => product.id === itemId);
      // if (deleteProduct) {
    ...
    <div className="flex min-[500px]:flex-row min-[500px]:items-center gap-5 py-6  border-b border-gray-200 group">
      ...
      <div className="w-[40%] md:max-w-[126px]">
        <Image
          ...
          className={`mx-auto skeleton-loading rounded-xl ${
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 ">
        <div className="md:col-span-2">
          <div className="flex flex-col  w-full md:items-start md:pl-4 md:gap-3 md:mb-0 mb-2 md:justify-left justify-center ">
            <h6
              className={`font-semibold text-base md:text-center text-left leading-7 text-black ${
                productsLoading && "skeleton-loading"
              }`}
            >
              {product.data.title}
            ...
            <h6
              className={`font-normal md:text-center text-left text-base leading-7 text-gray-500 ${
                productsLoading && "skeleton-loading"
              }`}
            >
            ...
            <h6
              className={`font-medium text-base leading-7 text-gray-600 transition-all md:text-center text-left duration-300 group-hover:text-red-800 ${
                productsLoading && "skeleton-loading"
              }`}
            >
        ...
        <div className="flex items-center justify-between max-[500px]:justify-center h-full max-md:mt-3 max-[500]:mb-4">
          <div className="flex items-center md:w-[70%] w-[50%]">
            <button
              className="group rounded-l-xl  px-2 md:py-[12px] py-3 border border-gray-200 flex items-center justify-center shadow-sm shadow-transparent transition-all duration-500 hover:bg-gray-50 hover:border-gray-300 hover:shadow-gray-300 focus-within:outline-gray-300"
              onClick={() => {
