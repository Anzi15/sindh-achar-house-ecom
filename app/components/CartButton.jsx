...
function CartButton({ notification = 0, disabled = false }) {
  return (
    <Link
    href={"/cart"}
      className={`
        h-6 aspect-square relative
    ...
    >
      ...
      {notification > 0 && (
        <div
          className={`
            h-[18px] aspect-square rounded-full px-1 py-0.5 text-xs absolute top-[-10px] right-[-7px] flex items-center justify-center
            ${disabled && "bg-neutral-100 text-neutral-400"}
            ${!disabled && "bg-indigo-700 text-white"}`
          }
        >
          {notification}
        </div>
